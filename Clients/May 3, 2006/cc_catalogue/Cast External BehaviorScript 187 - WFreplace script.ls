...
on displayPattern me
  ...
  member("WFreplace").image = myimg
  member("WFreplace").regPoint = point(0, 0)
end

on setwalls me, pattern, color
  iWallPatternIndex = pattern
  iWallColorIndex = color
end

on setfloor me, pattern, color
  iFloorPatternIndex = pattern
  iFloorColorIndex = color
end
