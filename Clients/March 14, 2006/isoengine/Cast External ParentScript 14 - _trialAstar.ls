...
on findPath me, _oN1, _oN2
  ...
  repeat while me.aOpen.count > 0
    ...
    repeat with i = 1 to iLength
      ...
      me.aAStarMap[nn.pt.locH][nn.pt.locV] = nn
    end repeat
    me.debug("STEP 2.5")
    n.s = n.closed
    me.aAStarMap[n.pt.locH][n.pt.locV] = n
  end repeat
  me.iElapsedTime = the milliSeconds - iStartTime
  return VOID
end
...
on getDirectPath me, pt1, pt2
  ...
  if pt1.locH = pt2.locH then
    ...
    repeat with i = 1 to iDif
      ...
      oMapNode = me.oMap.getNode(pt1.locH, iNextV)
      if voidp(oMapNode) then
        return VOID
      end if
      oNextPt = point(pt1.locH, iNextV)
      bEndNode = oNextPt = pt2
      if not me.getPassable(oMapNode, 0) then
        return VOID
      end if
...
on getSuccessors me, n, pt2
  ...
  repeat with i = 1 to me.iDirLength
    ...
    oMapNode = me.oMap.getNode(nextPt.locH, nextPt.locV)
    if oMapNode = VOID then
      next repeat
    end if
    bEndNode = oMapNode.equals(me.oEndNode)
    if not me.getPassable(oMapNode, bEndNode) then
      next repeat
    end if
    nn = me.aAStarMap[nextPt.locH][nextPt.locV]
    if nn = VOID then
  ...
  if not voidp(oMapNode) then
    ...
    if iNumPointers > 0 then
      repeat with i = 1 to iNumPointers
        ...
        oPointerMapNode = me.oMap.getNode(oPNode.row, oPNode.col)
        if not me.getPassable(oPointerMapNode, bEndNode) then
          next repeat
        end if
        nn = me.aAStarMap[nextPt.locH][nextPt.locV]
        if voidp(nn) then
...
on getPassable me, oMapNode, bEndNode
  ...
  if bSeatableItem and not bEndNode then
    return 0
  end if
