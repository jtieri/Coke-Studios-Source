...
on mouseUp me
  ...
  if member("nav_modify_studio_name").text = EMPTY then
    ...
    put "Calling oStudioManager.createStudio(" && sScreenName & "," & sStudioName & "," & sDescription & "," & iLayoutID & ",", iLocationID && ")"
    oStudioManager.createStudio(sScreenName, sStudioName, sDescription, iLayoutID, iLocationID)
  end if
end
