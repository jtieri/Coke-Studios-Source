...
on triggerPossessionAction me, iActionID, posId
  me.foRoom.triggerPossessionAction(iActionID, posId)
end
...
on receiveRemovePossession me, oCaller, iPosId, bDeleted
  ...
  put "about to get oPossession"
  oPossession = oIsoScene.getItemByPossessionId(iPosId)
  put "oPossession grabbed"
  if not voidp(oPossession) then
    put "oPossession not void"
    if bDeleted = "false" then
      put "bDeleted is false"
      if (oIsoScene.oSelectedItem.isFurniItem() or oIsoScene.oSelectedItem.isWallItem()) and oStudio.isOwner(oDenizenManager.getScreenName()) then
        put "not working:" && me && oIsoScene.oSelectedItem
        me.addPossessionToBackpack(oIsoScene.oSelectedItem, 1)
        oIsoScene.oSelectedItem = VOID
        oIsoScene.oInfoStand.display(oIsoScene.oSelectedItem)
      end if
    end if
    put "about to remove oPossession from scene"
    oIsoScene.deleteItem(oPossession)
    put "removed oPossession from scene"
    if not voidp(oIsoScene.oAvatars) then
      put "oIsoScene.oAvatars isn't void"
      oIsoScene.oAvatars.updateAll()
      put "updated all oIsoScene.oAvatars"
    end if
  else
  end if
end
...
on receiveTeleport me, oCaller, iLocation, iLayout, sStudioID, sStudioName, iDestinationPosId
  ...
  put "receiveTeleport() iLocation: " & iLocation & ", iLayout: " & iLayout & ", sStudioId: " & sStudioID & ", sStudioName: " & sStudioName & ", iDestinationPosId: " & iDestinationPosId
  ...
  if sCurrentStudioId = sStudioID then
    ...
    if integer(iDestinationPosId) <> 0 then
  ...
  if integer(iDestinationPosId) <> 0 then
    ...
    put "sent oStudioMap.setEnterStudioViaTeleport(" & iDestinationPosId & ")"
  else
    oStudio.sendEnterStudio()
    put "sent oStudio.sendEnterStudio()"
  end if
  if sStudioID starts "$SYSTEM$" then
    put "calling doTrackingCokeStudios(" && sStudioName && ")in Browser"
    sParamString = "javascript:doTrackingCokeStudios('cokestudios.room.enter', '" & sStudioName & "')"
