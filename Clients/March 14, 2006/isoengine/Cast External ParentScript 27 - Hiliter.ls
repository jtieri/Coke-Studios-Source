...
on updatePosition me, oSquare
  ...
  bRollover = 0
  bMouseIsOverBackpack = ElementMgr.mouseIsOverBackpack()
  if bMouseIsOverBackpack then
    me.dragFurniPreview()
    return 
  end if
  ...
  bTradingOpen = not voidp(ElementMgr.getTrader())
  if not bRollover or bTradingOpen then
    me.hide()
    me.dragFurniPreview()
    return 
  end if
  ...
  if (oSquare.iRow <> me.iRow) or (oSquare.iCol <> me.iCol) then
    if bRollover then
      me.show(oSquare)
      if not bTradingOpen then
        oIsoScene.updateSelectedItem(oSquare)
      end if
    else
      me.hide()
    end if
