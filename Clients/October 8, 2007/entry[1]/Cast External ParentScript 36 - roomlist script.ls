...
on new me, numwidth, numheight, whichparent
  ...
  if member("roomdisplay").memberNum < 1 then
    ...
    if mytext <> EMPTY then
      ...
      put "pRoomData:" && ilk(pRoomData) && pRoomData
      if not voidp(pRoomData) then
        pTotalLines = pRoomData.count()
...
on createimg me
  ...
  repeat with n = 1 to pTotalLines
    ...
    sStudioID = myroom.studioId
    iStudioID = integer(sStudioID.char[sStudioID.length - 1..sStudioID.length])
    if (iStudioID > 34) and (iStudioID < 48) then
      me.debug("createimg: iStudioID: " && iStudioID)
      me.debug("listmember.line.count-1: " && listmember.line.count - 1)
      me.debug("createimg: listmember.line[listmember.line.count-1]: " && listmember.line[listmember.line.count - 1])
      aAuditionRoomLines.add(listmember.line.count - 1)
      me.debug("aAuditionRoomLines: " && aAuditionRoomLines)
    end if
  ...
  repeat with n = 1 to listmember.text.lines.count
    ...
    myroom = pRoomData[n]
    lsStudioID = myroom.studioId
    liStudioID = integer(lsStudioID.char[lsStudioID.length - 1..lsStudioID.length])
    if aAuditionRoomLines.getOne(n) <> 0 then
      me.debug("createimg:listmember.line[n]: " && listmember.line[n])
      listmember.line[n].item[1].fontStyle = [#bold, #underline]
      listmember.line[n].item[2].fontStyle = [#bold, #underline]
      listmember.line[n].item[1].color = rgb("0000FF")
      listmember.line[n].item[2].color = rgb("0000FF")
      next repeat
    end if
    if liStudioID = 48 then
      listmember.line[n].item[1].fontStyle = [#bold, #underline]
      listmember.line[n].item[2].fontStyle = [#bold, #underline]
      listmember.line[n].item[1].color = rgb("FF0000")
      listmember.line[n].item[2].color = rgb("FF0000")
      next repeat
    end if
...
on displayRoomDetail me, roomdetail
  ...
  if voidp(circleanim) = 0 then
    ...
    maplocs = [#london: point(108, 15), #miami: point(46, 31), #mombasa: point(142, 49), #newyork: point(54, 24), #rio: point(72, 63), #seattle: point(22, 21), #sydney: point(224, 70), #tokyo: point(213, 27), #goa: point(162, 32), #alaska: point(7, 12), #moscow: point(136, 15), #redroom: point(23, 32), #sanfrancisco: point(19, 27), #centralpark: point(54, 24), #neworleans: point(38, 30), #mexico: point(33, 38), #neptune: point(79, 35), #castingcallroom: point(23, 32), #auditionBLUE: point(23, 32), #auditionGOLD: point(23, 32), #auditionGREEN: point(23, 32), #auditionORANGE: point(23, 32), #auditionRED: point(23, 32), #secretroom: point(54, 24), #comingsoonvenue: point(54, 24), #neworleans: point(38, 24)]
  ...
  if not voidp(roomdetail.description) then
