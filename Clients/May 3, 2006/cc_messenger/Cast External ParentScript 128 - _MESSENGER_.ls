...
on updateData me, _iFriendCount, _iEnemyCount, _iInviteCount, _iTotalMessageCount, _aFriends, _aEnemies, _aInvites, _aMessages
  if me.bDebug then
    ...
    put "_aFriends: " && ilk(_aFriends) & _aFriends
...
on displayWindow me, MyWindow, oArg
  ...
  if not voidp(MyWindow) then
    ...
    if the debugPlaybackEnabled then
      put "sWindowName:" && ilk(sWindowName) && sWindowName
    end if
    case sWindowName of
      "console_myinfo":
      ...
      "console_compose":
        lComposePropList = oArg.duplicate()
...
on displayCurrentWindow me, oArg
  ...
  case sWindowName of
    "console_compose":
      if voidp(oArg.mailto) then
        oArg = lComposePropList
      end if
      me.displayCompose(MyWindow, oArg)
    otherwise:
...
on displayMyInfo me, MyWindow
  ...
  sMissionStateMent = oDenizenManager.getMissionStatement()
  if voidp(sMissionStateMent) then
    sMissionStateMent = EMPTY
  end if
  member("missionfield").text = sMissionStateMent
...
on updateFriendsStatus me, aOnlineFriends
  if the debugPlaybackEnabled then
    put "_MESSENGER_::updateFriendsStatus"
  end if
  repeat with i = 1 to me.aFriends.count()
    aFriend = me.aFriends[i]
  ...
  if not voidp(MyWindow) then
    recipients = []
    if ilk(MyWindow.pScrollingLists) = #propList then
      if MyWindow.pScrollingLists.count > 0 then
    ...
    if recipients = [] then
      pCustomData = lComposePropList.duplicate()
    else
      pCustomData = [#mailto: recipients, #camefrom: "friends"]
    end if
    put "recipients:" && ilk(recipients) && recipients
