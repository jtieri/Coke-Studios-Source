...
on new me, sID, _iScale, _sAttributes, _sScreenName, _sMission, _bBot
  ...
  if oStudioMap.getLocation() = 18 then
    sFishyString = fishifyAvatarString(_sAttributes)
    me.oEngine = new(script("AvatarEngine"), sFishyString, _iScale)
  else
...
on mouseDownEvent me, iSpriteNum, bDoubleClick
  ...
  bIsMe = me.getScreenName() = oDenizenManager.getScreenName()
  if not bIsMe then
    oMyAvatar = oIsoScene.oAvatars.getAvatar(oDenizenManager.getScreenName())
    if not voidp(oMyAvatar) then
      oMyAvatar.faceAvatar(me)
    end if
  end if
  return bHitTest
end
...
on faceAvatar me, oAvatar
  ...
  oSquare1 = me.getSquare()
  oSquare2 = oAvatar.getSquare()
  iDir = oIsoScene.oGrid.getDirectionToSquare(oSquare1, oSquare2)
  me.debug("faceAvatar() iDir: " & iDir & ": " & oAvatar.getScreenName())
  if voidp(iDir) then
    return 
  end if
  if not voidp(oStudio) then
    oStudio.sendUpdateAvatar(VOID, VOID, VOID, iDir, VOID)
  end if
end
...
on nextPath me
  callAncestor(#nextPath, me)
end

on stopPathAnimation me
  callAncestor(#stopPathAnimation, me)
...
on updateStatus me
  ...
  if not bIsSeatable then
    ...
    oStandOn = oIsoScene.oFurniture.getStandOnAtSquare(me.oSquare)
    if voidp(oStandOn) then
      me.bOnStandOn = 0
    else
      iDir = oStandOn.getDirection()
      me.faceDirection(iDir)
      if me.bOnStandOn = 0 then
        me.bOnStandOn = 1
        oStandOn.oAction.standOn()
      end if
    end if
  end if
...
on debug me, sMessage, bForce
  me.ancestor.debug("isoengine::Avatar:: " & sMessage, bForce)
end
...
on hearChat me, oFromAvatar
  iMaxLookDistance = 3.5
  iDistance = integer(oIsoScene.oGrid.getDistanceBetweenSquares(oFromAvatar.getSquare(), me.getSquare()))
  if iDistance <= iMaxLookDistance then
    iMyDirection = me.oEngine.getDirection()
    iMyAngle = (iMyDirection * 45) - 90
    if iMyAngle < 0 then
      iMyAngle = 360 + iMyAngle
    end if
    fMathObj = newObject("Math")
    iRowDif = oFromAvatar.getSquare().getRow() - me.getSquare().getRow()
    iColDif = oFromAvatar.getSquare().getCol() - me.getSquare().getCol()
    iAngleToChatter = fMathObj.atan2(iRowDif, iColDif) / fMathObj.pi * 180
    iAngleToChatter = iAngleToChatter mod 360
    if iAngleToChatter < 0 then
      iAngleToChatter = 360 + iAngleToChatter
    end if
    iRelAngleToChatter = iAngleToChatter - iMyAngle
    if iRelAngleToChatter < -180 then
      iRelAngleToChatter = iRelAngleToChatter + 360
    end if
    if iRelAngleToChatter > 180 then
      iRelAngleToChatter = iRelAngleToChatter - 360
    end if
    if (iRelAngleToChatter >= -90) and (iRelAngleToChatter <= 90) then
      if (iRelAngleToChatter >= -30) and (iRelAngleToChatter <= 30) then
        iTurnDir = 0
      else
        iTurnDir = integer(iRelAngleToChatter / abs(iRelAngleToChatter))
      end if
      case iTurnDir of
        1:
          me.oEngine.lookRight(1)
        (-1):
          me.oEngine.lookLeft(1)
        0:
          me.oEngine.lookStraight(1)
      end case
    else
      return 
    end if
