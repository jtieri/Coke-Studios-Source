...
on new me, _sProdId, _sPosId, _sImageBase, _sType, _sAction, _iState, _aAttributes, _sCastLib, _iRow, _iCol, _iGridY, _iDirection, _iHeight, _iHeightOffset, _bItemsAllowedOnTop
  me.bDebug = 0
  me.debug("new( ) _sProdId: " & _sProdId & ", _sPosId: " & _sPosId & ", _sImageBase: " & _sImageBase & ", _sType: " & _sType & ", _sAction: " & _sAction, 0)
  me.debug("new FurniITem() 1")
  me.sProdID = _sProdId
  me.sPosId = _sPosId
  ...
  me.direction = 5
  me.frame = 6
  me.debug("new FurniITem() 2")
  me.loadAlias()
  me.debug("new FurniITem() 3")
  me.loadAssets()
  me.debug("new FurniITem() 4")
  me.SetWidth()
  me.debug("new FurniITem() 5")
  me.setDepth()
  me.debug("new FurniITem() 6")
  me.setNumFrames()
  me.debug("new FurniITem() 7")
  me.setDirections()
  me.debug("new FurniITem() 8")
  me.setDirectionIndex()
  me.debug("new FurniITem() 9")
  me.setDirection(me.iDirection)
  me.debug("new FurniITem() 10")
  me.setDrawOrder()
  me.debug("new FurniITem() 11")
  me.bDragging = 0
  me.bAnimate = 0
  me.iFrame = 0
  me.iFPS = 8
  ...
  me.debug("new FurniITem() 12")
  return me
end

on duplicate me
  return new(script("FurnitureItem"), sProdID, sPosId, sImageBase, sType, sAction, iState, aAttributes, sCastLib, iRow, iCol, iGridY, iDirection, iHeight, iHeightOffset, bItemsAllowedOnTop)
end
...
on mouseDownEvent me, iSpriteNum, bDoubleClick
  put "sprite" && iSpriteNum && "received a mousedown event"
  me.debug("FURNI MOUSE DOWN: " & me.sPosId)
  iSpriteNum = me.hitTestAll()
  bHitTest = not voidp(iSpriteNum)
  me.debug("*** FURNITURE ITEM: MOUSEDOWN: " & me.sImageBase & " HIT TEST: " & bHitTest & " ***")
  ...
  mousesquare = oIsoScene.oMouseSquare
  case iDirection of
    0:
      destSquare = oIsoScene.oGrid.getSquareByRowCol(oSquare.iRow, mousesquare.iCol)
    6:
      destSquare = oIsoScene.oGrid.getSquareByRowCol(mousesquare.iRow, oSquare.iCol)
  end case
  if voidp(bPass) or (bPass and not me.getDrag()) then
    oIsoScene.moveAvatar(destSquare)
  end if
  return 1
end
...
on placeItem me, oSquare
  ...
  if (aAvatars.count > 0) and not bIsSeatableItem then
    return 0
  end if
...
on drop me
  me.debug("drop()")
  if not voidp(ElementMgr.getTrader()) then
    return 0
  end if
...
on draw me
  ...
  repeat with i = 1 to me.aDrawOrder.count
    ...
    put sOrder && "depth =" && iZ
    newZ = me.getDepthByLayer(sOrder, oSquare)
    put sOrder && "newdepth = " & newZ
    sprite(iSprite).locZ = newZ
...
on setNumFrames me
  me.debug("setNumFrames() aAssets: " & me.aAssets)
  iHighestFrame = 0
...
on setFrame me, _iFrame
  ...
  me.iFrame = _iFrame
  me.debug("setFrame() me.iFrame: " & me.iFrame)
  me.draw()
end
...
on rotate me, iDir
  ...
  bSuccess = me.placeItem(oSquare)
  if bSuccess or (me.sAction contains "STAND_ON") then
    me.iLastDirectionIndex = me.iDirectionIndex
    oIsoScene.oAvatars.updateAll()
  end if
  return bSuccess
...
on getDepthByLayer me, sOrder, oSquare
  myorder = sOrder
  mysquare = oSquare
  _iRow = me.iRow
  _iCol = me.iCol
  case sOrder of
    "a":
      ...
      mysquare = oIsoScene.oGrid.getSquareByRowCol(_iRow, _iCol)
    "b":
      case me.iDirection of
        0:
          _iCol = me.iCol + 1
    ...
    "e":
      ...
      myorder = "a"
    "f":
      mysquare = me.getSquareByGreatestDepth()
      myorder = "a"
  end case
  mydepth = mysquare.getDepthFromMap(myorder, me.iGridY)
  return mydepth
end
